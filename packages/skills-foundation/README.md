...
> Vendored from the original [agents-workspace](https://github.com/wcgomes/agents-workspace) project. This package is the foundation meta-skill layer of the harness-template monorepo. It integrates with [agency-agents](https://github.com/msitarzewski/agency-agents) — 144+ specialized agents trusted by **86k+ developers**. The 5 foundational skills here sit on top.
...
| Mechanism | Skill | Command | What it does |
|---|---|---|---|
| **Delegation** | `delegate` | `/aw:delegate` | Discovers specialists, selects the best fit, dispatches structured handoffs, and reviews delegated work. |
| **Self-learning wiki** | `wiki` | `/aw:wiki` | Reads `wiki/index.md` before broad exploration, then adds, updates, removes, and lints workspace knowledge after tasks. Tracks skill candidates. |
| **Minimal changes** | `implement` | `/aw:implement` | Validates understanding before acting. Writes the least code that solves the problem. |
| **Systematic debugging** | `debug` | `/aw:debug` | Investigates errors with observe-hypothesize-verify-fix-confirm cycle. |
| **Skill authoring** | `agents-skills` | `/aw:agents-skills` | Creates, refines, and validates Agent Skills following the agentskills.io spec. |
...
**Run the installer locally from this package:**
```bash
./tools/install.sh
```
...
skills/                # Loadable behavioral rules — install globally
  delegate/            # Discovery, selection, handoff, review, fallback   (/aw:delegate)
  wiki/                # Wiki query and self-learning loop                  (/aw:wiki)
  implement/           # Validation and minimal changes                     (/aw:implement)
  debug/               # Systematic error investigation                     (/aw:debug)
  agents-skills/       # Skill authoring and validation                     (/aw:agents-skills)
...
All commands defined by this package use the `/aw:` prefix to avoid colliding with the spine commands provided by `harness-core`. Skill folder names and `name:` frontmatter values are unchanged from the upstream `agents-workspace` project — only the invocation aliases are namespaced.
