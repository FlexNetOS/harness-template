...
- **Public surface synced to the live repo** — metadata, catalog counts, plugin manifests, and install-facing docs now match the actual OSS surface: 68 agents, 182 skills, and 74 commands.
...
**That's it!** You now have access to 68 agents, 189 skills, and 74 commands.
...
For a reproducible dev environment that "just works" on Windows, macOS, and Linux without manually installing Node 20, Yarn 4.9.2, Python 3.11, and Playwright system deps, the harness ships a `.devcontainer/`. Open in VS Code and click "Reopen in Container", or use the `devcontainer` CLI.

**One-shot smoke (no VS Code needed):**
...
**VS Code (Windows/macOS/Linux host):** open the folder, then Command Palette → "Dev Containers: Reopen in Container". The post-create step installs deps, pre-warms MCP server caches, and runs `npm run verify`.

The container ships with Node 20 (Bookworm), Yarn 4.9.2, Python 3.11, the Claude Code CLI, and Playwright Chromium system libs. Secrets come from a host-side `.env` (gitignored) loaded via `runArgs --env-file`. See ADR 0004 (`docs/architecture/adr/0004-devcontainer-packaging.md`) for the full rationale and `.env.example` for required variables.
...
| Agents | 68 | `.cursor/agents/ecc-*.md` when installed; prefixed to avoid collisions with user or marketplace agents |
...
| Commands | PASS: 74 commands | PASS: 31 commands | **Claude Code leads** |
...
|---------|------------|------------|-----------|----------|
| **Agents** | 68 | Shared (AGENTS.md) | Shared (AGENTS.md) | 12 |
