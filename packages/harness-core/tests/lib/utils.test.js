...
// Hermetic git-repo helper: creates a fresh `git init` in a temp dir and
// runs `fn` with cwd switched into it. Always restores cwd and cleans up.
function withFreshGitRepo(fn) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecc-git-test-'));
...
// Hermetic non-git-dir helper.
function withFreshTmpDir(fn) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecc-nogit-test-'));
  const prev = process.cwd();
  process.chdir(dir);
  try {
    return fn(dir);
...
function runTests() {
  ...
  // isGitRepo() tests — hermetic: don't depend on the ambient CWD being a git repo
  console.log('\nisGitRepo():');

  if (test('isGitRepo returns true inside a fresh git repo', () => {
    withFreshGitRepo(() => {
      assert.strictEqual(utils.isGitRepo(), true);
    });
  })) passed++; else failed++;

  if (test('isGitRepo returns false outside any git repo', () => {
    withFreshTmpDir(() => {
      assert.strictEqual(utils.isGitRepo(), false);
    });
  })) passed++; else failed++;
